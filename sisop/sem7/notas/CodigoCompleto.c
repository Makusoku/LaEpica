...
void obtenerCadenaCluster(const char *image_file,exFatBootSector boot, int first_cluster){
    int bytes_per_sector = 1 << boot.BytePerSector,fd;
...
void readRootDir(const char *image_file){
    ...
    while(read(fd, entry, SIZE) == SIZE){
        ...
        if((unsigned char)entry[0] == 0xc0){
            ...
            capacidad = (unsigned char)entry[24]| 
                        ((unsigned char)entry[25] << 8 )| 
                        ((unsigned char)entry[26] << 16 );
        }
            ...
            //if(strcmp(name,"leeClustersFAT32.c.jpg")==0){
                printf("\nArchivo %s con tamaño %ld bytes:\n",name,capacidad);
